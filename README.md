...
2. **imdb**: `imdb/the matrix` goes straight to the title when IMDb's search suggestions make it clear: the only exact title match, or the most popular result if that is an exact match. A trailing year narrows it (`imdb/dune 1984`). Anything else, including errors or a 1.5 s timeout on IMDb's unofficial suggestion endpoint, goes to IMDb's search results. With a Chrome site search of keyword `imdb` → `https://nakom.is/imdb/%s`, typing `imdb inception` opens the film
